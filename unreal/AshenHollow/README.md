...
Os ataques alternam duas montagens no DefaultSlot, com mistura de entrada/saída e retorno à locomoção. As cópias em `/Game/AshenHollow/Animation` contêm um `AH_MeleeImpact` por golpe: 0,40 s em MM_Attack_01 e 0,4667 s em MM_Attack_02. Esses pontos correspondem à máxima extensão frontal dos punhos em amostras de 30 Hz, uma calibração inicial que ainda requer revisão visual de contato. O notify aplica o dano uma única vez; se não chegar, o final da animação serve de segurança. Golpes futuros sem notify usam 35% da duração. A alternância visual não usa o gerador de dados. Morte, perda de alcance ou obstrução podem cancelar o dano pendente; a animação não concede deslocamento gratuito.
...
Ainda é um duelo de teste com uma das quatro fichas predefinidas e um inimigo. Os manequins, animações desarmadas e blocos de cenário são provisórios; não são arte ou animações de Baldur's Gate 3, nem uma entrega de qualidade AAA final. A arma representada pelas regras ainda não tem modelo correspondente. Os golpes atuais têm AnimNotify; a calibração visual ainda é provisória. Faltam reações, ataques de oportunidade, cobertura, elevação, magias, múltiplos integrantes, exploração fora de turnos, criador UMG e habilidades GAS concretas. O inimigo tem um limite de dez segundos para não bloquear indefinidamente o encontro se não encontrar uma rota.
