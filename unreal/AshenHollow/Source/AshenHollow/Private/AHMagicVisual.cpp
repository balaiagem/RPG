...
AAHMagicVisual::AAHMagicVisual()
{
    ...
    // Three heads, each with six shrinking trail samples. No lights or collision.
...
void AAHMagicVisual::Initialize(const FVector& Origin,AActor* Target,float TravelSeconds)
{
...
void AAHMagicVisual::Tick(float DeltaSeconds)
{
    ...
    for(int32 I=0;I<Darts.Num();++I)
    {
        ...
        const FVector Arc=Side*((Dart-1)*95.f)+FVector(0,0,55.f+Dart*25.f);
        ...
        Darts[I]->SetVisibility(Age/Duration>=Trail*.028f);
    }
}
